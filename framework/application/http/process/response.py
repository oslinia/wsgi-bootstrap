...
class Http(Head):
    __slots__ = 'size', 'mimetype', 'encoding', 'block_size'
...
class Media(Http):
    ...
    def __init__(self, file: str):
        ...
        self.iana(*mimetypes.guess_type(file, strict=True))

    def error(self, body: bytes, start_response: StartResponse) -> bytes:
        self.size = len(body)

        self.iana('text/plain', 'ascii')
...
class Document(Http):
    ...
    def __init__(
            self, body: bytes | str, *,
            code: int = 200, mimetype: str = 'text/plain', encoding: str = None
    ):
